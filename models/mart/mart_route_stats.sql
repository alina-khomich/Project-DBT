with 
departures as (
	select origin, count (*) as departure_total from  prep_flights
	where cancelled = 0
	group by origin),
arrivals as (
	select dest, count (*) as arrival_total from  prep_flights
	where cancelled = 0
	group by dest), 
scheduled as (
...
unique_airplains as (
	...
	from 
		(select origin as airport, count( distinct tail_number) as unique_airplains from prep_flights
		group by airport
		union all
		select dest as airport, count( distinct tail_number) as unique_airplains from prep_flights
		group by airport) as t
	group by t.airport),
unique_airline as (
	select airport, avg(unique_airline)  as avg_unique_airline
	from 
		(select origin as airport, count( distinct airline) as unique_airline from prep_flights
		group by airport
		union all
		select dest as airport, count( distinct airline) as unique_airline from prep_flights
		group by airport) as t
	group  by airport)
select 
s.airport,
